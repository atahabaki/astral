...
pub fn get_defaults() -> ([u8;4], u16) {
    let addr = match std::env::var("ASTRAL_ADDR") {
        ...
        Err(_) => {
            eprintln!("WARN: ASTRAL_ADDR env var unset, using default: `{:?}`.", ASTRAL_ADDR);
            ASTRAL_ADDR
        },
    };
    let port = match std::env::var("ASTRAL_PORT") {
        Ok(port) => match port.parse::<u16>() {
            Ok(port) => port,
            Err(_) => {
                eprintln!("ERR: ASTRAL_PORT env var conversion failed, using default: `{}`.", ASTRAL_PORT);
                ASTRAL_PORT
            },
        },
        Err(_) => {
            eprintln!("WARN: ASTRAL_PORT env var unset, using default: `{}`.", ASTRAL_PORT);
            ASTRAL_PORT
        },
    };
    (addr, port)
